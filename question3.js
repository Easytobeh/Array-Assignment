...
function webDevsOnly(arr) {

    let webDevsOnly = [];
    let nigerianWebDevs = [];
    let belowTwentyYears = [];
    let activeEmployees = [];
    let highestSalary = 0;
    let highestPaidDev = '';
    ...
    arr.forEach(element => {
        ...
        if (element.Job === 'Web Developer')
        {
            webDevsOnly.push(element); //find all developers

            //Find highest paid developer
            if (highestSalary < element.salary)
                highestPaidDev = element.first_name
            else
                highestPaidDev = highestPaidDev;
        }
          
        //A web developer and a Nigerian
        if (element.Job === 'Web Developer' && element.Country === 'Nigeria')
            nigerianWebDevs.push(element);
        
        if (element.age < 20)
            belowTwentyYears.push(element);
    ...
    console.log("webDevelopers = ", webDevsOnly);
    console.log("nigerianWebDevs = ", nigerianWebDevs);
    console.log("belowTwentyYears = ", belowTwentyYears);
    console.log("activeEmployees = ", activeEmployees);
    console.log("highest Paid Developer: ", highestPaidDev);

}

webDevsOnly(employees);